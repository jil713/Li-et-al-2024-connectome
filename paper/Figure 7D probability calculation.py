...
for i, df_n in enumerate(dfs):
    file_name = f'probability_{i}.xlsx'
    clusterlabel=[1]*24+[2]*27+[3]*35+[4]*29+[5]*40
    df_n['clusterlabel'] = clusterlabel
    df_cluster = df_n.set_index('clusterlabel')

    cluster1=df_cluster.iloc[0:24,:]
    cluster2=df_cluster.iloc[24:51,:]
    cluster3=df_cluster.iloc[51:86,:]
    cluster4=df_cluster.iloc[86:115,:]
    cluster5=df_cluster.iloc[115:155,:]
    ...
    for column5 in cluster5.columns:
        num_entries_clus5 = cluster5[column5].count()
        clus5counts_list.append(num_entries_clus5)
    ...
    for n in clus5counts_list:
        combination = math.comb(n,2)
        list_cluster5.append(combination)

    comb_sumeachcluster = [sum(x) for x in zip(list_cluster1, list_cluster2, list_cluster3, list_cluster4, list_cluster5)]
